...
while typed != text:  # Only stop when the typed text matches the given text
    if msvcrt.kbhit():
        char = msvcrt.getwch()  # getwch() supports Unicode
        if char == '\r':  # Enter key to continue to the next line (optional)
            continue
        elif char == '\x08':  # Backspace
            typed = typed[:-1]  # Remove the last character from the typed string
            print("\b \b", end='', flush=True)  # Visual backspace (clear character on screen)
        else:
